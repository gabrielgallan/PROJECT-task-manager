import { INestApplication } from '@nestjs/common'
import { Test } from '@nestjs/testing'
import { addDays } from 'date-fns'
import request from 'supertest'
import { UUIDGenerator } from 'test/e2e/factories/uuid-generator'
import { SessionTokenGenerator } from '@/domain/identity/application/cryptography/session-token-generator'
import { SessionTokenHasher } from '@/domain/identity/application/cryptography/session-token-hasher'
import { AppModule } from '@/infra/app.module'
import { SESSION_COOKIE_NAME } from '@/infra/auth/session-cookie'
import { PrismaService } from '@/infra/database/prisma/prisma.service'

describe('Edit plan schedule [E2E]', () => {
	let app: INestApplication
	let prisma: PrismaService
	let sessionTokenGenerator: SessionTokenGenerator
	let sessionTokenHasher: SessionTokenHasher

	let userId: string
	let planId: string
	let sessionToken: string

	beforeAll(async () => {
		const moduleRef = await Test.createTestingModule({
			imports: [AppModule],
		}).compile()

		app = moduleRef.createNestApplication()

		prisma = moduleRef.get(PrismaService)

		sessionTokenGenerator = moduleRef.get(SessionTokenGenerator)

		sessionTokenHasher = moduleRef.get(SessionTokenHasher)

		await app.init()

		userId = UUIDGenerator(1)[0]
		planId = UUIDGenerator(1)[0]
		sessionToken = sessionTokenGenerator.generate()

		await prisma.user.create({
			data: {
				id: userId,
				name: 'John Doe',
				email: 'johndoe@email.com',
				jobTitle: 'Developer',
				sessions: {
					create: {
						tokenHash: sessionTokenHasher.hash(sessionToken),
						expiresAt: addDays(new Date(), 30),
					},
				},
				plans: {
					create: {
						id: planId,
						title: '[REVIEW]: Review Auto Guide',
						startsAt: new Date(2026, 0, 12, 9, 0, 0),
						endsAt: new Date(2026, 0, 12, 10, 0, 0),
					},
				},
			},
		})
	})

	it('[PATCH] /api/plans/:planId/schedule', async () => {
		await request(app.getHttpServer())
			.patch(`/api/plans/${planId}/schedule`)
			.set('Cookie', `${SESSION_COOKIE_NAME}=${sessionToken}`)
			.send({
				startsAt: new Date(2026, 0, 13, 14, 0, 0).toISOString(),
				endsAt: new Date(2026, 0, 13, 16, 0, 0).toISOString(),
			})
			.expect(204)

		const plan = await prisma.plan.findUnique({ where: { id: planId } })

		expect(plan?.startsAt).toEqual(new Date(2026, 0, 13, 14, 0, 0))
		expect(plan?.endsAt).toEqual(new Date(2026, 0, 13, 16, 0, 0))
		expect(plan?.title).toBe('[REVIEW]: Review Auto Guide')
	})

	it('[PATCH] /api/plans/:planId/schedule returns 404 for a plan owned by another user', async () => {
		const [otherUserId, otherPlanId] = UUIDGenerator(2)

		await prisma.user.create({
			data: {
				id: otherUserId,
				name: 'Jane Doe',
				email: 'janedoe@email.com',
				jobTitle: 'Designer',
				plans: {
					create: {
						id: otherPlanId,
						title: '[DOCS]: Update APP docs',
						startsAt: new Date(2026, 0, 12, 9, 0, 0),
						endsAt: new Date(2026, 0, 12, 10, 0, 0),
					},
				},
			},
		})

		await request(app.getHttpServer())
			.patch(`/api/plans/${otherPlanId}/schedule`)
			.set('Cookie', `${SESSION_COOKIE_NAME}=${sessionToken}`)
			.send({
				startsAt: new Date(2026, 0, 13, 8, 0, 0).toISOString(),
				endsAt: new Date(2026, 0, 13, 9, 0, 0).toISOString(),
			})
			.expect(404)
	})

	afterAll(async () => {
		await app.close()
	})
})
