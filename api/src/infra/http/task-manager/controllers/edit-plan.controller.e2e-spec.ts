...
describe('Edit plan [E2E]', () => {
	...
	it('[PATCH] /api/plans/:planId', async () => {
		...
		expect(plan?.title).toBe('[REVIEW]: Review Auto Guide - part 1')
		expect(plan?.endsAt).toEqual(new Date(2026, 0, 12, 11, 30, 0))
		expect(plan?.taskId).toBe(taskId)
		expect(plan?.categoryId).toBe(categoryId)
	})

	it('[PATCH] /api/plans/:planId clears a relation sent as null', async () => {
		await request(app.getHttpServer())
			.patch(`/api/plans/${planId}`)
			.set('Cookie', `${SESSION_COOKIE_NAME}=${sessionToken}`)
			.send({
