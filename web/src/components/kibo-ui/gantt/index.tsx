...
export const GanttSidebarHeader: FC<GanttSidebarHeaderProps> = ({ label = 'Issues' }) => (
	<div
...
export const GanttSidebar: FC<GanttSidebarProps> = ({ children, className, label }) => (
	<div
		className={cn(
...
export type GanttProviderProps = {
	...
	children: ReactNode
	className?: string
	/** Opens the timeline around this date instead of centering on its own data. */
...
export const GanttProvider: FC<GanttProviderProps> = ({
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: "positions the timeline once, on mount, with the initial geometry"
	useEffect(() => {
		...
		if (initialScrollDate) {
			// The sidebar is measured by a later effect, but the scroll math needs it
			// now, so it is resolved the same way here: present means 300px.
			const initialSidebarWidth = scrollElement.querySelector('[data-roadmap-ui="gantt-sidebar"]')
				? 300
				: 0

			const offset = getOffset(initialScrollDate, new Date(timelineData[0].year, 0, 1), {
				zoom,
				range,
				columnWidth,
			...
			// The offset is relative to the timeline track, which starts after the
			// sidebar column, so only the visible part of the track is centered.
			scrollElement.scrollLeft = Math.max(
				0,
				offset - (scrollElement.clientWidth - initialSidebarWidth) / 2,
			)
		} else {
	...
	return (
		...
		>
			<div
				className={cn(
					'gantt relative isolate grid h-full w-full flex-none select-none overflow-auto rounded-0 bg-secondary',
					range,
					className,
				)}
				ref={scrollRef}
...
export const GanttToday: FC<GanttTodayProps> = ({ className }) => {
	...
	return (
		...
		>
			<div
				className={cn(
					'group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-b-md bg-red-400 px-2 py-1 text-white text-xs',
					className,
				)}
			>
				{label}
			...
			<div className={cn('h-full w-px bg-red-400', className)} />
		</div>
	)
}
