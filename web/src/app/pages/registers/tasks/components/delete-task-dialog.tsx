...
export function DeleteTaskDialog({ task, onOpenChange, onDeleted }: {
	task: Task | null; onOpenChange: (open: boolean) => void; onDeleted: (task: Task) => void
}) {
	if (!task) return null
	return <OpenDeleteTaskDialog key={task.id} task={task} onClose={() => onOpenChange(false)} onDeleted={onDeleted} />
}
function OpenDeleteTaskDialog({ task, onClose, onDeleted }: { task: Task; onClose: () => void; onDeleted: (task: Task) => void }) {
	const mutation = useDeleteTask()
	const { capture, busy, ended } = useIdentityLifecycle()
	const { clearTask: clearPlanTask } = usePlans()
	...
	async function confirm() {
		...
		try {
			...
			if (!current()) return
			clearPlanTask(task.id); clearWorkLogTask(task.id)
			toast.success('Task deleted', { description: 'Associated plans and work logs are now unassigned.' })
			onDeleted(task); onClose()
		} catch (failure) {
		...
		} finally { locked.current = false; if (current()) setPending(false) }
	}
	function close() { if (!locked.current && !disabled) onClose() }
	return <AlertDialog open onOpenChange={(open) => !open && close()}>
		<AlertDialogContent>
			<AlertDialogHeader>
				<AlertDialogMedia><Trash2 className="text-destructive" /></AlertDialogMedia>
				<AlertDialogTitle>Delete task</AlertDialogTitle>
				<AlertDialogDescription>“{task.title}” will be removed. Plans and work logs pointing to it are kept.</AlertDialogDescription>
			</AlertDialogHeader>
			{error && <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}
			<AlertDialogFooter>
				<AlertDialogCancel disabled={disabled}>Cancel</AlertDialogCancel>
				<AlertDialogAction variant="destructive" onClick={() => void confirm()} disabled={disabled || unavailable}>{pending ? 'Deleting…' : 'Delete'}</AlertDialogAction>
			</AlertDialogFooter>
		</AlertDialogContent>
	</AlertDialog>
}
