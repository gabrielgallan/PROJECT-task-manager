...
export function useTasksPage() {
	...
	const list = useTasksQuery({ ...filters, page: query.page, limit: TASKS_PAGE_SIZE }, view === 'list')
	const collection = useTasksQuery(filters, view !== 'list')
	...
	async function changeStatus(task: Task, status: TaskStatus) {
		...
		if (!result.success) { setActionError(result.error.issues[0]?.message ?? 'Select a valid status'); return }
		setActionError(null)
		try {
			...
			if (!(error instanceof TaskActionBlockedError)) setActionError(getTaskError(error, 'status'))
		}
	}
	async function reschedule(task: Task, startDate: Date, dueDate: Date) {
		...
		try {
			...
			if (!(error instanceof TaskActionBlockedError)) setActionError(getTaskError(error, 'schedule'))
			return false
		}
	}
	...
	return {
		view, setView, query, filters: filterDraft, list, collection, currentQuery, tasks, meta,
		result: { tasks, total: meta?.total ?? 0, page: meta?.page ?? query.page, pageCount },
		editingTask, setEditingTask, deletingTask, setDeletingTask, detailedTask, setDetailedTask,
		actionError, clearActionError: () => setActionError(null), details, openCreateDialog,
		changeStatus, reschedule, openPlan, openWorkLog,
		filtered: hasActiveTaskFilters(query), toggleSort, setPage,
	}
}
